import { copyToClipboard } from "@/utility/otherUtility";
...
const ActivityComponent: React.FC<{
  ...
  const copyText = (e: any, str: string) => {
    e.preventDefault();
    copyToClipboard(str)
  }
  return (
    <div
      className={`activity ${isLast ? "last-activity" : ""
    ...
    >
      <div className="col-span-2">
        ...
        <div className="mt-10 location flex gap-4 items-center w-max">
          <a
            href={`https://maps.google.com/maps?q=${activity.name.split(" ").join("+")}`}
            target="_blank"
          ...
          <a
            ...
            onClick={(e) => {
              copyText(e, `https://maps.google.com/maps?q=${activity.name.split(" ").join("+")}`)
            }}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
