...
const Itinerary = (
  ...
  const [itinaryFullData, setItinaryFullData] = useState<itinaryWithImage | null>(dummyData)
  ...
  const dataConvert = async () => {
    if (itinerary) {
      const destinationImageUrl = await fetchImageFromKeyword(itinerary?.destination);
  ...
  return (
    <div>
      <div
        className="w-full h-screen bg-cover bg-center relative" style={{
        ...
        <div className="w-full h-full text-white backdrop-brightness-50 flex flex-col justify-between">
          ...
          <div className="flex flex-col gap-6 items-center justify-center">
            ...
            <h1 className="text-header font-bold leading-none -mt-10">{itinaryFullData?.destination}</h1>
          </div>
          <div className="days container mx-auto">
            <h6 className="font-bold text-4xl mb-16">{itinaryFullData?.numberOfDays} Days in Delhi</h6>
          </div>
        </div>
        <div className=" container mx-auto my-20">
          ...
          <p className="text-gray-700 mb-6 text-center">{itinaryFullData?.description}</p>
...
const ActivityComponent: React.FC<{ activity: itinaryWithImageActivity }> = ({ activity }) => (
